...
class Inst:
    def __init__(self, addr, block, size, mnem, op, moduleBase):
        self.addr = addr
        self.block = block
        self.size = size
        self.mnem = mnem
    ...
    def simple_str(self):
        inst_line = f"{int(self.addr, 16) - int(self.moduleBase, 16):X}\t{self.mnem}\t{self.op}"
        return inst_line
...
class Arm64Ctx:
    def __init__xxx(self, pc, sp,
        ...
        self.general_regs = [int(x0, 16), int(x1, 16), int(x2, 16), int(x3, 16), int(x4, 16), int(x5, 16), int(x6, 16), int(x7, 16), int(x8, 16), int(x9, 16),
                             int(x10, 16), int(x11, 16), int(x12, 16), int(x13, 16), int(x14, 16), int(x15, 16), int(x16, 16), int(x17, 16), int(x18, 16),
                             int(x19, 16),
...
class Arm64TraceLog:
    ...
    def on_message(self, payload):
        ...
        if type == 'inst':
            ...
            inst = Inst(val["address"], payload["block"], val["size"], val["mnemonic"], val["opStr"], payload["moduleBase"])
