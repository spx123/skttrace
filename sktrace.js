...
(() => {
    ...
    recv("config", (msg) => {
        ...
        if (payload.spawn) {
            console.log(`-----spawn traceTarget-----`);
            hook_dlopen(libname, traceTarget(libname, payload))
        } else {
            console.log(`-----attach traceTarget-----`);
            traceTarget(libname, payload);
        }
    })
})()
...
function traceTarget(libname, payload) {
    return function () {
